...
def load_builtin_history_projects():
    ...
    data = [
       ...
       {"项目名称":"乌坭浪","项目子类型":"出售类","总建筑面积":58380,"总投资":68397.84,"售价":12880,"租金":0,"residential_area":0,"comm_area":1750,"park_count":584,"occupancy_stable":0.90,"comm_occupancy_stable":0.90,"park_occupancy_stable":0.90,"manage_coeff":1.92,"land_cost":1308.71,"construction_cost":50240.70,"infra_cost":875.77,"sale_area":56105,"comm_rent_start_price":64,"park_rent_start_price":200,"park_income_ratio":0.50},
       {"项目名称":"创景苑","项目子类型":"出租类","总建筑面积":34330,"总投资":50928.27,"售价":0,"租金":19.2,"residential_area":34330,"comm_area":0,"park_count":470,"occupancy_stable":0.90,"comm_occupancy_stable":0.9,"park_occupancy_stable":0.90,"manage_coeff":1.92,"land_cost":12000,"construction_cost":37828.07 ,"infra_cost":420,"sale_area":0,"comm_rent_start_price":28,"park_rent_start_price":200,"park_income_ratio":0.50},
       #{"项目名称":"历史出租项目B","项目子类型":"出租类","总建筑面积":62000,"总投资":59000,"售价":0,"租金":22.0,"residential_area":54000,"comm_area":3200,"park_count":580,"occupancy_stable":0.93,"comm_occupancy_stable":0.86,"park_occupancy_stable":0.91,"manage_coeff":2.00,"land_cost":15000,"construction_cost":6200,"infra_cost":500,"sale_area":0,"comm_rent_start_price":30,"park_rent_start_price":320,"park_income_ratio":0.55},
        {
            "项目名称": "历史租售结合项目A",
            "项目子类型": "租售结合类",
            "总建筑面积": 56000,
...
4. 总成本费用(不含建设期财务费用、不含税金) = 经营成本 + 财务费用(运营期) +折旧摊销=（管理费用+空置期物业管理费+日常物业维修基金+保险费用+装修重置费+税金及附加）+财务费用(运营期) +折旧摊销
5. 财务费用来自还本付息表中的本期付息
     其中:①财务费用＝财务费用(建设期)+财务费用(运营期)
          ②还本付息表按照迭代:本期计息＝(期初借款＋本期借款/2)*3%→本期还息＝本期计息(每期利息均还完)→本期还本=总借款×3%（以后每年为前年的1+3%（1+4.5%）^n倍数，n=1,2,3...）→期末借款累计=期初借款＋本期借款＋本期计息-本期还本-本期还息
6. 净现金流量 = 现金流入 - 现金流出合计
7. 净现值(NPV):∑净现金流/(1+3.5%)^(0.5+n),n=0、1、2....（第一年为0.5，以后每年递增）  即按折现率对各年净现金流量逐年折现后求和
8. 全周期内部收益率(IRR):按全周期净现金流序列迭代求解
9. 弥补亏损:第一次利润总额为正时，弥补首次盈利前5年亏损（若无法完全弥补前面5年亏损，最长顺延5年
10.应纳税所得额=∑（利润总额+弥补亏损）
""".strip()
...
10.净利润 = 利润总额 - 所得税  
11.增值税=∑（住宅租金×[1.5%/（1+5%）]+车位租金×[9%/（1+9%）]）（其他收入不算增值税）  
12.印花税=∑（收入×[0.05%/（1+9%）]）  
13.城镇维护建设税=∑（增值税×7%）  
14.教育附加和地方教育附加税=∑（增值税×5%）  
15.房产税=∑（住宅租金×出租率×[4%/（1+5%）]+车位租金×[12%/（1+9%）]+建安工程费×[70%×1.2%/（1+9%）]×（1-出租率）×租赁月数/12） （①前三年免征 ②不要加权，各年计算后求和 ③建安工程费）  
16.城镇土地使用税=用地面积×3  
17.净现金流量= 现金流入 - 现金流出=收入- （建设投资+税金及其附加+管理费用+空置期物业管理费+维修费用+保险费用+装修重置费+日常物业维修基金+所得税）  
18.利息保障倍数=EBIT/(建设期财务费用+经营期财务费用)=(借款期利润总额+经营期财务费用)/(建设期财务费用+经营期财务费用)  

""".strip()

    sale_text = """
【出售类补充公式】
1. 配保房销售收入 = =∑销售面积×平均售价/10000=∑配保房销售面积×销售率×平均售价/10000
2. 租金收入=∑(商业租金收入-出租经营税金-出租营运成本）/(1+3.5%)^n
  ①商业出租收入:∑（商业出租面积×出租率×租金×12/10000+车位个数×月租金×12/10000
  ②出租营运成本=房产税1+房产税2+运营管理费用（商业）+运营管理费用（停车场）+物业专项维修金+空置物业服务费+保险费用+土地使用税
（1）房产税1=∑商业出租收入×[12%/（1+9%）]
（2）房产税2=∑[土地成本费＋建安工程费（非配售）＋基础设施建设费+工程建设其他费用（非配售）+建安工程费（非配售）×2%×（1-配保房面积/(配保房面积＋商业面积)）]×70%×1.2%×（1-出租率）
（3）运营管理费用（商业）=∑商业出租收入×8%
（4）运营管理费（停车场）=∑车位个数×80×12/10000
（5）物业专项维修金=∑商业面积×出租率×租赁月数×0.25
（6）维修费用=∑租金收入×0.02
（7）空置物业服务费=∑出租面积×（1-出租率）×8%×12×0.88/10000
（8）保险费用=∑出租面积×1.86/10000
（9）土地使用税=∑项目用地面积×商业出租面积/(配保房销售面积+商业出租面积)×3/10000
  ③出租净收入 = 商业出租收入 - 出租营运成本合计 - 出租经营税金合计
  ④出租净收益现值 = 将各年出租净收入按现值规则折现后汇总
  
 出租经营税金=增值税(一般计税)+增值税附加+印花税
  ①增值税(一般计税)以进项税和销项税做判断：如果销项税-进项税>0，则输出销项税-进项税；否则输出0
（1）进项税:
合计=（各年累计管理费用＋各年累计保险费）×[6%/（1+6%）]＋合计空置物业服务费×[9%/（1+9%）]＋工程进项税
注：工程进项税=建安工程费/计容建筑面积×900×9%/1.09+工程建设其他费用/计容建筑面积×900×6%/1.06
建设和运营期：如前一年进项税-今年销项税>0，则输出前一年进项税-今年销项税；否则输出0（建设期首年的前一年为合计）
（2）销项税：合计=各年累计租金收入×[9%/（1+9%）]
建设和运营期=∑租金收入×[9%/（1+9%）]
  ②增值税附加=∑增值税(一般计税)×12%
  ③印花税=∑租金收入×0.05%
3. 出售类总收入 = 配保房销售收入 + 出租净收益现值 + 住宅租金收入 + 车位收入 + 其他收入
4.总成本费用计算：总成本费用=开发成本（销售部分）+开发成本（折旧摊销）+销售费用+销售税金+财务费用
5.累计开发成本（销售部分）=总投资-资本成本-销售费用=总投资-建设期财务费用×配保房销售面积/(配保房销售面积+商业出租面积)-配保房销售收入×1.5%
6.累计开发成本（折旧摊销部分）=（非配售开发成本（含地价）-商业资金成本）×0.8=[非配售开发成本（含地价）-建设期财务费用×（1-配保房销售面积/(配保房销售面积＋商业面积）]×0.8
7.销售费用 = 当年配保房销售收入 × 1.5%
8. 销售税金及其附加 = 增值税 + 增值税附加 + 印花税
   ①增值税：
（1）地价减抵款=配保房销售面积×划拨土地楼面价
（2）增值税销项税款=∑（当期销售款-地价减低款×销售率）×[9%/（1+9%）]
（3）增值税进项税款=∑（工程建设其他费用（合计）+销售费用）×销售率×[6%/（1+6%）]+（建安工程费＋基础设施建设费）×销售率×[9%/（1+9%）]
                 =∑（非配售工程建设其他费用×(配保房销售面积+商业出租面积)/商业出租面积+合计销售费用）×销售率×[6%/（1+6%）]+（合计建安工程费＋合计基础设施建设费）×(配保房销售面积+商业出租面积)/商业出租面积+销售费用）×销售率×[9%/（1+9%）]
（5）增值税：以进项税和销项税做判断：如果当年增值税销项税款-增值税进项税款>0，则增值税销项税款-增值税进项税款；否则输出0
   ②增值税附加=∑增值税×12%
   ③印花税＝当期销售款×[印花税率/（1+9%）]
9. 出售类利润总额 = 总收入 - 总成本费用= 经营收入-开发成本（销售部分）-开发成本（折旧摊销部分）-销售费用-销售税金及其附加-财务费用（建设期）-财务费用（运营期）
10.出售类现金流入=配保房销售收入+其他收入+商业出租收入+回收固定资产余值
  ①回收固定资产余值=[（（非配售）土地成本费＋（非配售）开发成本费-（非配售）资本成本）]×0.2=[（非配售）土地成本费＋（非配售）开发成本费-建设期财务费用×商业面积/(配保房面积＋商业面积)]×0.2
11. 出售类现金流出合计 = 开发成本投资 + 销售费用 + 销售税金及附加 + 出租经营税金 + 出租营运成本 + 调整所得税
  ①调整所得税=∑[（现金流入-回收固定资产余值）-（累计开发成本（销售部分）+累计开发成本（折旧摊销部分）+销售费用+销售税金+出租营运成本＋出租经营税金]×0.25
  ②开发成本投资=总投资×配保房面积/(商业面积＋配保房面积)-建设期财务费用-销售费用
12. 利息保障倍数(出售类) = (净利润合计 + 经营期财务费用 + 所得税 + 累计开发成本(折旧摊销部分) - 0.8×其他收入) ÷ (建设期财务费用 + 经营期财务费用)
                       =  (净利润+经营期财务费用+所得税+累计开发成本(折旧摊销部分)-0.8×其他收入)/(建设期财务费用+经营期财务费用)
13.利润总额=收入-总成本费用-税金及其附加
14.净利润=利润总额-所得税（营业利润→净利润相同）
15.净现金流=现金流入-现金流出
16.净现值=∑净现金流/(1+3.5%)^(n),n=0、1、2....（第一个非负现金流年，以后每年递增）
17.全投资内部收益率=IRR(第一年净现金流量到最后一年现金流量)
""".strip()
...
if run_manual_inputs:
    ...
    with st.expander("2. 收入计算参数", expanded=True):
        ...
        if "sale_and_commercial" in current_config.get("ui_components", []):
            ...
            sale_avg_price = col_sale2.number_input("售价（元/㎡）", min_value=0.0, max_value=999999.0, value=12880.0, step=100.0)
...
def calc_operating_cost(all_years, month_dict, is_operate, resi_area, resi_occupancy, resi_rent_price, park_income_list, total_build_area, manage_coeff, decoration_cost, house_type, total_investment, operate_year_list):
    ...
    for year in all_years:
        if not is_operate[year]:
            ...
            # 各项经营成本（原有逻辑完全不变）
            manage_house = resi_area * occ * 12 * 1.92 * manage_coeff  / 10000
   ...
   # 2c. 运营费用
    ...
    for year in all_years:
            if not is_operate[year]:
                ...
                vat = max(output_tax - input_before, 0.0)
                vat_surcharge = vat * 0.12
                stamp = (nr_decoration_unit_cost * residential_area / 10000 * 0.0003) if year == operate_year_list[0] else 0.0  # 首次装修成本×0.03%/只算一次
...
if calc_button or has_result_snapshot_for_current_page(current_page_key):
    try:
       ...
       # 【最小改动：仅加if判断，分项目类型处理】
        ...
        # ===================== 【替换】利息保障倍数计算（分项目类型，出售类用新公式）=====================
        if project_type == "出售类(配保房/可售型人才房等)":
            # 出售类：直接取各表格合计值，严格按新公式计算
            net_profit_total = profit_df["净利润(万元)"].sum()
            operate_fin_cost = total_cost_df["财务费用(运营期)(万元)"].sum()
            income_tax_total = profit_df["所得税(万元)"].sum()
            dev_cost_dep_total = total_cost_df["累计开发成本（折旧摊销部分）(万元)"].sum()
            other_income_total = income_df[f"{other_income_name}(万元)"].sum()
            build_fin_cost = total_cost_df["财务费用(建设期)(万元)"].sum()
            
            # 分子：净利润 + 经营期财务费用 + 所得税 + 累计开发成本(折旧摊销部分) - 0.8×其他收入
            numerator = net_profit_total + operate_fin_cost + income_tax_total + dev_cost_dep_total - 0.8 * other_income_total
            # 分母：建设期财务费用 + 经营期财务费用
            denominator = build_fin_cost + operate_fin_cost
            # 除0兜底
            interest_coverage_ratio = round(numerator / denominator, 2) if denominator != 0 else 0.0
        else:
            # 非出售类：保持原有逻辑完全不变
